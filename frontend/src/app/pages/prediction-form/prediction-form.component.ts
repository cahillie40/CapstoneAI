...
export class PredictionFormComponent implements OnInit {
  ...
  savePrediction(): void {
    if (!this.selectedPlayerId || !this.predictionResult) {
      return;
    }
    ...
    this.predictionService.savePrediction(this.selectedPlayerId, this.request, this.predictionResult).subscribe({
      next: () => {
        this.saving = false;
